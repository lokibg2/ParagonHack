...
$uname = $_SESSION['siD'];	
	...
	if($_POST['answer'] == '9:41' || $_POST['answer'] == '09:41'){
		echo "<script>
		
		alert(\"Bingo!!! That's right! :) \");</script>";
	$mysqli->query("UPDATE user SET level = 4,time= CURRENT_TIMESTAMP WHERE uname='$uname' ");
	
	}else{
		if($_SESSION['level'] != 4){
		echo "<script>alert(\"Sorry, that's a wrong answer\");</script>";
		echo "<script>window.location.assign('Level3.php');</script>";
		}
	}
...
Great job in clearing this level 3 <br>
We want more pariticipants on this level. So please wait for more contestants to join you.<br>
Kindly be patient for more hacking experience and check this site soon!!<br><br>
